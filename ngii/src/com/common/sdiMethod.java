...
public class sdiMethod {
	...
	public void folderDeletePath(String FolderPath)
	{
		...
		try {
			for(File targetFile : files)
			{
				...
				{
					folderDeletePath(targetFile.getAbsolutePath());
				}
				else if(targetFile.isFile())
				{
					int pos = targetFile.getName().lastIndexOf(".");
					if(!targetFile.getName().substring(pos+1).equals("zip"))
					{
						targetFile.delete();
					}
				}
